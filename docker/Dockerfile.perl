...
# Install CPAN modules using only dependency manifests so this layer survives
# normal source changes.
#
WORKDIR /app
WORKDIR /app/docker.template
...
# Entrypoint script for starting process. Kicks off the selected WebDyne server on ${PORT}
#
COPY docker/entrypoint.sh /usr/local/bin
ENTRYPOINT ["/usr/local/bin/entrypoint.sh"]


# Empty cmd, entrypoint script runs the default PSGI/Starman server unless WEBDYNE_SERVER selects another runtime.
#
CMD []
