...
C<use WebDyne::Cache ...> is intended to be called from within a WebDyne page C<__PERL__> block. When imported successfully it marks the page as static and stores the nominated cache callback in page metadata.
...
Via use of the WebDyne::Cache module, supplying a code ref as an import parameter as in the synopsis:
...
Via use of the meta-data field, with supply of the code ref in the E<lt>headE<gt> section of the document, e.g.
...
Via use of the cache attribute in the start_html tag:
...
Internal chaining handler used when the module is inserted into the WebDyne handler pipeline. It reads the cache handler name from C<WebDyneCacheHandler>, marks the page static, and passes control back to the main WebDyne handler flow.
